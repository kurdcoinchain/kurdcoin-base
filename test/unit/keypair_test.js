describe('Keypair.contructor', function() {
  it('fails when passes secret key does not match public key', function() {
    ...
    let kp = KurdCoinBase.Keypair.fromSecret(secret);

    let secretKey = kp.rawSecretKey();
    let publicKey = KurdCoinBase.StrKey.decodeEd25519PublicKey(kp.publicKey());
    publicKey[0] = 0; // Make public key invalid

    expect(
      () => new KurdCoinBase.Keypair({ type: 'ed25519', secretKey, publicKey })
  ...
  it('fails when secretKey length is invalid', function() {
    ...
    expect(
      () => new KurdCoinBase.Keypair({ type: 'ed25519', secretKey })
  ...
  it('fails when publicKey length is invalid', function() {
    ...
    expect(
      () => new KurdCoinBase.Keypair({ type: 'ed25519', publicKey })
...
describe('Keypair.fromSecret', function() {
  it('creates a keypair correctly', function() {
    ...
    expect(kp).to.be.instanceof(KurdCoinBase.Keypair);
  ...
  it("throw an error if the arg isn't strkey encoded as a seed", function() {
    expect(() => KurdCoinBase.Keypair.fromSecret('hel0')).to.throw();
    expect(() =>
      KurdCoinBase.Keypair.fromSecret(
        'SBWUBZ3SIPLLF5CCXLWUB2Z6UBTYAW34KVXOLRQ5HDAZG4ZY7MHNBWJ1'
      )
    ).to.throw();
    expect(() =>
      KurdCoinBase.Keypair.fromSecret('masterpassphrasemasterpassphrase')
    ).to.throw();
    expect(() =>
      KurdCoinBase.Keypair.fromSecret(
...
describe('Keypair.fromRawEd25519Seed', function() {
  it('creates a keypair correctly', function() {
    ...
    let kp = KurdCoinBase.Keypair.fromRawEd25519Seed(seed);
  ...
  it("throws an error if the arg isn't 32 bytes", function() {
    expect(() =>
      KurdCoinBase.Keypair.fromRawEd25519Seed('masterpassphrasemasterpassphras')
    ).to.throw();
    expect(() =>
      KurdCoinBase.Keypair.fromRawEd25519Seed(
        'masterpassphrasemasterpassphrase1'
      )
    ).to.throw();
    expect(() => KurdCoinBase.Keypair.fromRawEd25519Seed(null)).to.throw();
    expect(() => KurdCoinBase.Keypair.fromRawEd25519Seed()).to.throw();
  });
});

describe('Keypair.fromPublicKey', function() {
  it('creates a keypair correctly', function() {
    let kp = KurdCoinBase.Keypair.fromPublicKey(
      'GAXDYNIBA5E4DXR5TJN522RRYESFQ5UNUXHIPTFGVLLD5O5K552DF5ZH'
    );
    expect(kp).to.be.instanceof(KurdCoinBase.Keypair);
    expect(kp.publicKey()).to.eql(
  ...
  it("throw an error if the arg isn't strkey encoded as a accountid", function() {
    expect(() => KurdCoinBase.Keypair.fromPublicKey('hel0')).to.throw();
    expect(() =>
      KurdCoinBase.Keypair.fromPublicKey('masterpassphrasemasterpassphrase')
    ).to.throw();
    expect(() =>
      KurdCoinBase.Keypair.fromPublicKey(
  ...
  it("throws an error if the address isn't 32 bytes", function() {
    ...
    expect(() => KurdCoinBase.Keypair.fromPublicKey(null)).to.throw();
    expect(() => KurdCoinBase.Keypair.fromPublicKey()).to.throw();
  });
});

describe('Keypair.random', function() {
  it('creates a keypair correctly', function() {
    let kp = KurdCoinBase.Keypair.random();
    expect(kp).to.be.instanceof(KurdCoinBase.Keypair);
  });
});
...
describe('Keypair.xdrMuxedAccount', function() {
  it('returns a valid MuxedAccount with a Ed25519 key type', function() {
    const kp = KurdCoinBase.Keypair.fromPublicKey(
      'GAXDYNIBA5E4DXR5TJN522RRYESFQ5UNUXHIPTFGVLLD5O5K552DF5ZH'
    );
    const muxed = kp.xdrMuxedAccount();
    expect(muxed).to.be.instanceof(KurdCoinBase.xdr.MuxedAccount);
    expect(muxed.switch()).to.be.equal(
      KurdCoinBase.xdr.CryptoKeyType.keyTypeEd25519()
    );
  });
});
