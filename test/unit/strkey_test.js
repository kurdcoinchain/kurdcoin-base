describe('StrKey', function() {
  beforeEach(function() {
    var keypair = KurdCoinBase.Keypair.master(
    ...
    this.seedEncoded = KurdCoinBase.StrKey.encodeEd25519SecretSeed(
  ...
  describe('#decodeCheck', function() {
    it('decodes correctly', function() {
      expect(
        KurdCoinBase.StrKey.decodeEd25519PublicKey(this.accountIdEncoded)
      ).to.eql(this.unencodedBuffer);
      expect(
        KurdCoinBase.StrKey.decodeEd25519SecretSeed(this.seedEncoded)
      ).to.eql(this.unencodedBuffer);
    });

    it('throws an error when the version byte is wrong', function() {
      expect(() =>
        KurdCoinBase.StrKey.decodeEd25519SecretSeed(
          'GBPXXOA5N4JYPESHAADMQKBPWZWQDQ64ZV6ZL2S3LAGW4SY7NTCMWIVL'
        )
      ).to.throw(/invalid version/);
      expect(() =>
        KurdCoinBase.StrKey.decodeEd25519PublicKey(
  ...
  describe('#encodeCheck', function() {
    it('encodes a buffer correctly', function() {
      expect(
        KurdCoinBase.StrKey.encodeEd25519PublicKey(this.unencodedBuffer)
      ).to.eql(this.accountIdEncoded);
      expect(
        KurdCoinBase.StrKey.encodeEd25519PublicKey(this.unencodedBuffer)
      ).to.match(/^G/);
      expect(
        KurdCoinBase.StrKey.encodeEd25519SecretSeed(this.unencodedBuffer)
      ...
      strkeyEncoded = KurdCoinBase.StrKey.encodePreAuthTx(this.unencodedBuffer);
      expect(strkeyEncoded).to.match(/^T/);
      expect(KurdCoinBase.StrKey.decodePreAuthTx(strkeyEncoded)).to.eql(
        this.unencodedBuffer
      );

      strkeyEncoded = KurdCoinBase.StrKey.encodeSha256Hash(this.unencodedBuffer);
      expect(strkeyEncoded).to.match(/^X/);
      expect(KurdCoinBase.StrKey.decodeSha256Hash(strkeyEncoded)).to.eql(
        this.unencodedBuffer
      );
    });
    ...
    it('throws an error when the data is null', function() {
      expect(() => KurdCoinBase.StrKey.encodeEd25519SecretSeed(null)).to.throw(
        /null data/
      );
      expect(() => KurdCoinBase.StrKey.encodeEd25519PublicKey(null)).to.throw(
        /null data/
      );
    });
  });
  ...
  describe('#isValidEd25519PublicKey', function() {
    it('returns true for valid public key', function() {
      ...
      for (var i in keys) {
        expect(KurdCoinBase.StrKey.isValidEd25519PublicKey(keys[i])).to.be.true;
      }
    });
    ...
    it('returns false for invalid public key', function() {
      ...
      for (var i in keys) {
        expect(KurdCoinBase.StrKey.isValidEd25519PublicKey(keys[i])).to.be.false;
      }
    });
  });
  ...
  describe('#isValidSecretKey', function() {
    it('returns true for valid secret key', function() {
      ...
      for (var i in keys) {
        expect(KurdCoinBase.StrKey.isValidEd25519SecretSeed(keys[i])).to.be.true;
      }
    });
    ...
    it('returns false for invalid secret key', function() {
      ...
      for (var i in keys) {
        expect(KurdCoinBase.StrKey.isValidEd25519SecretSeed(keys[i])).to.be
          .false;
      }
    });
  });
