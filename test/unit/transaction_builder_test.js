...
describe('TransactionBuilder', function() {
  describe('constructs a native payment transaction with one operation', function() {
    ...
    var memo;
    beforeEach(function() {
      source = new KurdCoinBase.Account(
      ...
      memo = KurdCoinBase.Memo.id('100');

      transaction = new KurdCoinBase.TransactionBuilder(source, {
        fee: 100,
  ...
  describe('constructs a native payment transaction with integer timebounds', function() {
    it('should have have timebounds', function(done) {
      ...
      let transaction = new KurdCoinBase.TransactionBuilder(source, {
        timebounds,
        fee: 100,
        networkPassphrase: KurdCoinBase.Networks.TESTNET
      })
  ...
  describe('setTimeout', function() {
    it('not called', function() {
      ...
      let transactionBuilder = new KurdCoinBase.TransactionBuilder(source, {
        fee: 100
      }).addOperation(
        KurdCoinBase.Operation.payment({
          destination:
  ...
  describe('.buildFeeBumpTransaction', function() {
    it('builds a fee bump transaction', function(done) {
      ...
      const innerSource = KurdCoinBase.Keypair.master(networkPassphrase);
      const innerAccount = new KurdCoinBase.Account(
      ...
      let innerTx = new KurdCoinBase.TransactionBuilder(innerAccount, {
        fee: '200',
      ...
      let feeSource = KurdCoinBase.Keypair.fromSecret(
        'SB7ZMPZB3YMMK5CUWENXVLZWBK4KYX4YU5JBXQNZSK2DP2Q7V3LVTO5V'
      );
      let transaction = KurdCoinBase.TransactionBuilder.buildFeeBumpTransaction(
        feeSource,
        '200',
        innerTx,
        networkPassphrase
      ...
      expect(transaction).to.be.an.instanceof(KurdCoinBase.FeeBumpTransaction);

      // The fee rate for fee bump is at least the fee rate of the inner transaction
      expect(() => {
        KurdCoinBase.TransactionBuilder.buildFeeBumpTransaction(
          feeSource,
          '100',
          innerTx,
          networkPassphrase
      ...
      innerTx = new KurdCoinBase.TransactionBuilder(innerAccount, {
        fee: '80',
        networkPassphrase: networkPassphrase,
        timebounds: {
          minTime: 0,
      ...
      const feeBumpTx = KurdCoinBase.TransactionBuilder.buildFeeBumpTransaction(
        feeSource,
        '200',
        innerTx,
        networkPassphrase
      ...
      const sourceAccountEd25519 = KurdCoinBase.Keypair.fromPublicKey(
        KurdCoinBase.StrKey.encodeEd25519PublicKey(
          v1Tx.sourceAccount().ed25519()
        )
      )
        .xdrAccountId()
        .value();
      const v0Tx = new KurdCoinBase.xdr.TransactionV0({
        ...
        ext: new KurdCoinBase.xdr.TransactionV0Ext(0)
      });
      const innerV0TxEnvelope = new KurdCoinBase.xdr.TransactionEnvelope.envelopeTypeTxV0(
        new KurdCoinBase.xdr.TransactionV0Envelope({
          tx: v0Tx,
      ...
      const feeBumpV0Tx = KurdCoinBase.TransactionBuilder.buildFeeBumpTransaction(
        feeSource,
        '200',
        new KurdCoinBase.Transaction(innerV0TxEnvelope, networkPassphrase),
        networkPassphrase
      );
  ...
  describe('.fromXDR', function() {
    it('builds a fee bump transaction', function() {
      ...
      let tx = KurdCoinBase.TransactionBuilder.fromXDR(
        xdr,
        KurdCoinBase.Networks.TESTNET
      );
      expect(tx).to.be.an.instanceof(KurdCoinBase.FeeBumpTransaction);
      expect(tx.toXDR()).to.equal(xdr);

      tx = KurdCoinBase.TransactionBuilder.fromXDR(
    ...
    it('builds a transaction', function() {
      ...
      expect(tx).to.be.an.instanceof(KurdCoinBase.Transaction);
      expect(tx.toXDR()).to.equal(xdr);
