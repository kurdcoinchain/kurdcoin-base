...
describe('FeeBumpTransaction', function() {
  beforeEach(function() {
    ...
    this.innerSource = KurdCoinBase.Keypair.master(this.networkPassphrase);
    this.innerAccount = new KurdCoinBase.Account(
    ...
    this.asset = KurdCoinBase.Asset.native();

    this.innerTx = new KurdCoinBase.TransactionBuilder(this.innerAccount, {
      fee: '100',
    ...
    this.feeSource = KurdCoinBase.Keypair.fromSecret(
      'SB7ZMPZB3YMMK5CUWENXVLZWBK4KYX4YU5JBXQNZSK2DP2Q7V3LVTO5V'
    );
    this.transaction = KurdCoinBase.TransactionBuilder.buildFeeBumpTransaction(
      this.feeSource,
      '100',
      this.innerTx,
  ...
  it('constructs a FeeBumTransaction object from a TransactionEnvelope', function() {
    ...
    expect(innerTransaction.memo.type).to.be.equal(KurdCoinBase.MemoText);
    ...
    const expectedTxEnvelope = KurdCoinBase.xdr.TransactionEnvelope.fromXDR(
      expectedXDR,
      'base64'
    ).value();
  ...
  it('throws when a garbage Network is selected', function() {
    ...
    expect(() => {
      new KurdCoinBase.FeeBumpTransaction(input, { garbage: 'yes' });
    }).to.throw(/expected a string/);

    expect(() => {
      new KurdCoinBase.FeeBumpTransaction(input, 1234);
    }).to.throw(/expected a string/);
  });
  ...
  it('adds signature correctly', function() {
    ...
    const addedSignatureTx = new KurdCoinBase.FeeBumpTransaction(
  ...
  it('adds signature generated by getKeypairSignature', function() {
    ...
    const signature = new KurdCoinBase.FeeBumpTransaction(
      transaction.toEnvelope(),
      this.networkPassphrase
  ...
  it('does not add invalid signature', function() {
    ...
    const alteredTx = KurdCoinBase.TransactionBuilder.buildFeeBumpTransaction(
      this.feeSource,
      '200',
      this.innerTx,
      this.networkPassphrase
  ...
  it('outputs xdr as a string', function() {
    const xdrString =
      '';
    const transaction = new KurdCoinBase.FeeBumpTransaction(
      xdrString,
      this.networkPassphrase
    );
    expect(transaction).to.be.instanceof(KurdCoinBase.FeeBumpTransaction);
  ...
  it('handles muxed accounts', function() {
    ...
    const txWithMuxedAccount = new KurdCoinBase.FeeBumpTransaction(
      envelope,
      this.networkPassphrase
    );
